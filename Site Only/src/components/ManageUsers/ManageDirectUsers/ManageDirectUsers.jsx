...
class ManageDirectUsers extends React.Component {
    _isMounted = false;
        ...
        axiosAccountsGET()
            ...
            .then(() => {
                activeUsers = users.filter((user) => {
                    return user.status === 'SUCCESS' && user.type === 'DIRECT';
                });
                pendingUsers = users.filter((user) => {
                    return user.status === 'PENDING' && user.type === 'DIRECT';
                });
                if (this._isMounted) {
                    this.setState({
                            ...
                            <div className="card">
                                ...
                                <div className="participants bg-white">
                                    <div className="row participants-header m-0 bg-ghost-white">
                                        ...
                                        {this.state.currentlyShownType !==
                                            'pending' && (
                                            <div className="col-5 box--flex center--xxX">
                                                Submitted on
                                            </div>
                                        )}
                                    </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(ManageDirectUsers);
